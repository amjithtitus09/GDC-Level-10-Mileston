...
class FunctionTest(TestCase):
    def setUp(self):
        ...
        self.user = User.objects.create_user(
            username="amjithtitus09", email="amjithtitus09@gmail.com", password="ascii123"
        )
...
class CelerySendReportsTest(TestCase):
    ...
    def test_get(self):
        ...
        self.user.save()
        before_count = (
            User.objects.filter(send_report_at__isnull=False, send_report_at__lte=datetime.datetime.now().time())
            .filter(Q(last_sent_on__isnull=True) | Q(last_sent_on__lt=datetime.datetime.today()))
            .count()
        )
        send_reports()
        after_count = (
